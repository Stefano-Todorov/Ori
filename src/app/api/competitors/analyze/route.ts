...
export async function POST(req: NextRequest) {
  ...
  const rl = checkRateLimit(`${user.id}:competitor-analyze`, RATE_LIMITS['competitor-analyze'])
  if (!rl.allowed) return NextResponse.json({ error: 'Too many requests' }, { status: 429, headers: getCorsHeaders(req) })
