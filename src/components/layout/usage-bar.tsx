...
export function UsageBar() {
  ...
  if (!data || data.tier === 'explorer') {
    // Free tier: show upgrade prompt
    return (
      <div className="p-3 border-t border-border">
        <Link
          href="/dashboard/settings?tab=billing"
          className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-gradient-to-r from-purple-600/10 to-purple-500/10 border border-purple-500/20 hover:border-purple-500/40 transition-all group"
        >
          <Sparkles size={14} className="text-purple-500 shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-foreground">Unlock AI Features</p>
            <p className="text-[10px] text-muted-foreground">Upgrade from $3.99/mo</p>
          </div>
        </Link>
      </div>
    )
    ...
    return (
      <div className="p-3 border-t border-border">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-purple-500/5 border border-purple-500/10">
          ...
          <span className="text-[11px] font-semibold text-purple-500">Studio</span>
