...
export function CoachChat({ initialHistory }: Props) {
  ...
  const [greetingLoaded, setGreetingLoaded] = useState(initialHistory.length > 0)
  ...
  // Proactive greeting — when there's no chat history, the coach speaks first
  const fetchGreeting = useCallback(async () => {
    if (greetingLoaded || streaming) return
    ...
    try {
      const res = await fetch('/api/coach/greeting', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      })

      if (!res.ok || !res.body) {
        // Fallback to static greeting if API fails (e.g. usage limit)
        setMessages([{
          role: 'assistant',
          content: "What can I help you with today? I know your niche, competitors, and goals — ask me anything about your content strategy.",
        }])
        setStreaming(false)
        return
      }
    ...
    setStreaming(false)
  }, [greetingLoaded, streaming])

  useEffect(() => {
    if (initialHistory.length === 0 && !greetingLoaded) {
      fetchGreeting()
    }
  }, [initialHistory.length, greetingLoaded, fetchGreeting])
